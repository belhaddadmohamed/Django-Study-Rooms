...
// Alert Notification # added by : Belhaddad mohamd islem
...
const createIconToast = (id, toast, nb) => {
    // Getting the icon and text for the toast based on the id passed
    const { icon, text } = toastDetails[id];

    document.getElementById(`icon_${nb}`).classList.add(`${icon}`);
...
for(let i=0; i<toasts.length; i++){
    ...
    // We set the flex direction : row-reversed so don't worry about the position of the icon 
    columns[i].innerHTML += `<i class="fa-solid" id="icon_${i}"> </i>`
    createIconToast(toast.id, toast, i)
}
